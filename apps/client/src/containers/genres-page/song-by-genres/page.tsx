...
export const SongByGenres: React.FC<Props> = ({ id }) => {
  const route = useRouter();
  const searchParams = useSearchParams();
  const pageString = searchParams.get('page');
  const currentPage = Number(pageString) || 1;

  const { data: dataSongByGenre, isLoading: isLoadingSongbyGenre } =
    useFetchListSong({
      q: '',
      genre: id,
      limit: 18,
  ...
  const totalPages = Math.ceil(
    dataSongByGenre?.pagination.total / dataSongByGenre?.pagination.limit,
  );
      ...
      <div className="mx-auto max-w-6xl">
        <div className="py-12 text-center">
          ...
          <p className="mx-auto max-w-2xl text-gray-600">
            Gồm {dataSongByGenre?.pagination?.total} bài hát cho thể loại {id}
          </p>
        </div>

        {/* Loading skeleton */}
        {isLoadingSongbyGenre && (
        ...
        {/* Songs list */}
        <ListSong data={dataSongByGenre?.data} />
